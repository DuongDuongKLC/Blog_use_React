...
export default class Content1 extends React.Component {
                ...
                <div class="item_main wcalc(calcpc350) h-100 float-right h-100">
                    <div class="title_item h(50) w-100 bdc-bt(Black) rounded">
                        <div class="title_child h(100pc) wcalc(calcpc200) float-left overflow-hidden">
                            <Link to={`/kali-linux/${status.stt_status}`} class="ff(Lemon) c(Black) fz(15) font-weight-bold float-left pt-sm-3 pl-5 hov(c-Black):h"> {status.title_status} </Link>
                        </div>
    ...
    render(){
        var content = this.state.data_status.filter((item)=> item.ma_status == 22)
